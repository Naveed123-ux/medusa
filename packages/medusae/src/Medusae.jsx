...
const Particles = ({ config }) => {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uOuterOscJitterStrength: {
        value: MEDUSAE_DEFAULTS.halo.outerOscJitterStrength,
      },
      uOuterOscJitterSpeed: { value: MEDUSAE_DEFAULTS.halo.outerOscJitterSpeed },
      ...
      new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                float outerJitterPhase = uTime * uOuterOscJitterSpeed + aRandom * PI * 2.0;
                float outerJitterNoise = noise(
                  dirToMouse * 3.2 + vec2(cos(outerJitterPhase), sin(outerJitterPhase)) * 0.9
                );
                float outerJitter = (outerJitterNoise - 0.5) * 2.0 * uOuterOscJitterStrength;
                float tentacleWobble = sin(
                  angleToMouse * 2.5 - uTime * (uOuterOscFrequency * 0.55 + uOuterOscJitterSpeed)
                  + aRandom * PI * 2.0
                ) * outerJitter * outerInfluence;
                pos.xy += radialDir * outerOsc * uOuterOscAmplitude * outerInfluence;
                pos.xy += tangentDir * tentacleWobble;
  ...
  useEffect(() => {
    ...
    material.uniforms.uOuterOscJitterStrength.value =
      merged.halo.outerOscJitterStrength;
    material.uniforms.uOuterOscJitterSpeed.value = merged.halo.outerOscJitterSpeed;
